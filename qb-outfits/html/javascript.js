...
$(document).on('click', '.deleteoutfit', function(e) {
    ...
    $.post('http://qb-outfits/DeleteOutfit', JSON.stringify({
        slot: outfitID,
    }));
});
...
$(document).on('click', '.selectoutfit', function(e) {
    ...
    $.post('http://qb-outfits/SelectOutfit', JSON.stringify({
        slot: outfitID,
    }));
});
...
$(document).on('click', '.saveoutfit', function(e) {
    ...
    if (outfitName.length > 2) {
        $.post('http://qb-outfits/CreateOutfit', JSON.stringify({
            slot: outfitID,
            name: outfitName,
        }));
    }
