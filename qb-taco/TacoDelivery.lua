...
Citizen.CreateThread(function()
	...
	Citizen.Wait(5)
        ...
        local Area = GetDistanceBetweenCoords(pos.x, pos.y, pos.z, v.x, v.y, v.z, false)
		if Area <= 2.3  and JobBusy == false and not IsPedInAnyVehicle(PlayerPedId(), false) then
		...
		elseif Area <= 3.0 and JobBusy == true then
			DrawText3D(v.x, v.y, v.z + 0.15, '~r~You\'re already working..\n ~s~Taco\'s Available: ~g~' ..Config.JobData['tacos']..'x')
		elseif Area <= 3.0 and IsPedInAnyVehicle(PlayerPedId(), false) then
...
AddEventHandler('qb-taco:start:black:job', function()
	...
	TriggerServerEvent('QBCore:Server:AddItem', "taco-bag", 1)
	TriggerEvent("inventory:client:ItemBox", QBCore.Shared.Items["taco-bag"], "add")
	if Tasks then
		return
	end
	Tasks = true
	while Tasks do
		Citizen.Wait(5)
		local pos = GetEntityCoords(PlayerPedId(), false)
		local Area = GetDistanceBetweenCoords(pos.x, pos.y, pos.z, DropOffs[rnd]["x"], DropOffs[rnd]["y"], DropOffs[rnd]["z"], false)
		if Area <= 3.0 then
...
RegisterNetEvent('qb-taco:client:set:taco:count')
AddEventHandler('qb-taco:client:set:taco:count', function(what, bool)
	Config.JobData[what] = bool
end)

RegisterNetEvent('qb-taco:client:set:register:count')
AddEventHandler('qb-taco:client:set:register:count', function(bool)
